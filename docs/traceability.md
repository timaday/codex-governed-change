...
| GOV-003 | T03, T24 | `test_candidate_identity.py`, untracked regular/symlink retained-descriptor and leaf-race tests, reviewer source-plus-snapshot and gate/mutation copy-local drift, recursive-submodule and concrete-mutant-tree tests, `test_reviewer_isolation.py` |
...
| GOV-021 | T04, T06, T09 | `test_gate_evidence.py`, shared gate/reviewer arbitrary-root POSIX/Windows/UNC host-path, named-endpoint and bare IPv4/IPv6 canaries, benign RFC3339 clock-value regression, unit gate-runner short-host/address endpoint canaries, `test_reviewer_isolation.py`, deterministic command-event omission plus final-message/raw-stream credential/endpoint/host-path canaries, permission-profile environment canary, `test_governance_integrity.py` |
...
| GOV-052 | T15, T20, T23 | `test_assurance_case.py` descriptor-bound parent/leaf swap/symlink/FIFO/oversize tests, general authoritative-JSON symlink/FIFO/oversize and command-session single-observation/conflicting-digest tests, reviewer finding missing-path/out-of-range-line/swapped-locator tests, artifact and reviewer-output retained-parent/leaf re-stat barriers, raw noncanonical reviewer-output preservation, split-checkout reference-workflow reviewer/admission command tests, protected-authority prompt reconstruction, end-to-end reviewer CLI protected-authority/candidate-root materialization and single-read race/special-file tests, `test_reviewer_isolation.py` |
...
| GOV-058 | T19 | `test_mutation_governance.py`, per-mutant source-before/copy/source-after identity and shared Git/tree deadline tests, producer-to-admission original-candidate/mutated-execution subject tests, protected-corpus bytes, recursive concrete mutated-tree identity, selected-fixture pre-mutation target isolation, exact template/record/gate/sandbox/executed command identity, structured selected-unittest causal proof and post-probe drift tests |
