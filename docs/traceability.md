...
| GOV-057 | T18, T24 | `test_rapid_review.py` protected evidence-locator substitution matrix, `test_rst_operations.py` complete exact relationship-graph, one-note-per-session/exact-oracle coverage, exact protected requirement/change/observation/surviving-mutant/finding update set, duplicate-reference, empty/extra feedback-edge and presence-only red controls, admission reconstruction |
